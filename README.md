...
- **RAM**: 24 GB recommended (16 GB minimum with 4-bit quantization)
- **Python**: 3.11+
...
# 3. Convert weights (downloads ~17GB from HuggingFace, ~7GB output)
...
|---|---|---|
| **DiT** | 4B MMDiT, 128 latent ch, 3072 hidden, 24 heads, 12 blocks | MLX `nn.Module`, 4-bit quantized |
| **Text Encoder** | Qwen3-VL (2560 hidden, 36 layers) | mlx-lm Qwen3-VL, 4-bit quantized |
...
- **Quantization**: 4-bit group quantization (group_size=64) for DiT + text encoder

## Memory Budget (24 GB MacBook Air M5)

| Component | 4-bit Quantized |
|---|---|
| DiT transformer | ~2.5 GB |
| Qwen3-VL text encoder | ~2-3 GB |
| MageVAE | ~0.5 GB |
...
├── convert_weights.py      # PyTorch → MLX conversion + 4-bit quantization
